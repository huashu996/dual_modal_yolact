...
class Yolact(nn.Module):
    ...
    def __init__(self):
        ...
        self.backbone1 = construct_backbone(cfg.backbone)
        self.backbone2 = construct_backbone(cfg.backbone)
        ...
        src_channels = self.backbone1.channels
        
        # For dual modal fusion in backbone
        self.middle_fusion_layer1 = nn.ModuleList([
            nn.Conv2d(x * 2, x, kernel_size=1)  #in_channels, out_channels, kernel_size
            #reversed逆序
            for x in reversed([src_channels[i] for i in self.selected_layers])
        ])
        
        self.middle_fusion_layer2 = nn.ModuleList([
        ...
        # Don't change the parameters in backbone? Why???
    ...
    def init_weights(self, backbone_path):
        ...
        self.backbone1.init_backbone(backbone_path)
        self.backbone2.init_backbone(backbone_path)
        ...
        for name, module in self.named_modules():
            ...
            if is_conv_layer and module not in self.backbone1.backbone_modules and module not in self.backbone2.backbone_modules:
    ...
    def forward(self, x):
        ...
        with timer.env('backbone'):
            if x.size(1) == 6:
                x = torch.split(x, [3, 3], dim=1)
                x1 = x[0] #图片1
                x2 = x[1] #图片2
            else:
                raise ValueError("x.size(1) should be 6, but got {}.".format(x.size(1)))
            outs1 = self.backbone1(x1)
            outs2 = self.backbone2(x2)
        #fpn
        if cfg.fpn is not None:
            with timer.env('fpn'):
                # Use backbone.selected_layers because we overwrote self.selected_layers
                outs1 = [outs1[i] for i in cfg.backbone.selected_layers] #每一层的输出
                outs2 = [outs2[i] for i in cfg.backbone.selected_layers]
#***************************************************
                gate = []
                x = torch.zeros(1, device=outs1[0].device)
                for i in range(len(outs1)):
                    gate.append(x)

                j = len(outs1) #层数
                for fusion_layer1 in self.middle_fusion_layer1:
                    j -= 1
                    gate[j] = torch.cat((outs1[j], outs2[j]), dim=1)
                    gate[j] = fusion_layer1(gate[j])

                j = len(outs1)
                for fusion_layer2 in self.middle_fusion_layer2:
                    j -= 1
                    gate[j] = fusion_layer2(gate[j])
                    gate[j] = torch.sigmoid(gate[j])

                outs = []
                for i in range(len(outs1)):
                    outs1[i] =      gate[i]  * outs1[i]
                    outs2[i] = (1 - gate[i]) * outs2[i]

                    max_outs = torch.max(outs1[i], outs2[i])
                    mean_threshold = torch.nn.functional.adaptive_avg_pool2d((outs1[i] + outs2[i]), output_size=(1, 1)) # [B, C, 1, 1]
                    outs.append(torch.where(max_outs > mean_threshold, max_outs * 2, (outs1[i] + outs2[i])))
#****************************************************
                outs = self.fpn(outs)
...
if __name__ == '__main__':
    ...
    x = torch.zeros((1, 6, cfg.max_size, cfg.max_size))
    y = net(x)
    
    print('net\n', net)

    print('\nnet.prediction_layers')
